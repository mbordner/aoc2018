...
import (
	...
	"github.com/mbordner/aoc2018/common/ranges"
	"regexp"
	"strconv"
)
...
func main() {
	...
	xRanges := make([]int, 0, len(nanobots)*2)
	yRanges := make([]int, 0, len(nanobots)*2)
	zRanges := make([]int, 0, len(nanobots)*2)
	for _, bot := range nanobots {
		xRanges = append(xRanges, []int{bot.pos.X - bot.radius, bot.pos.X + bot.radius}...)
		yRanges = append(yRanges, []int{bot.pos.Y - bot.radius, bot.pos.Y + bot.radius}...)
		zRanges = append(zRanges, []int{bot.pos.Z - bot.radius, bot.pos.Z + bot.radius}...)
	}

	xOverlaps := ranges.Overlaps(xRanges)
	yOverlaps := ranges.Overlaps(yRanges)
	zOverlaps := ranges.Overlaps(zRanges)

	fmt.Println(xOverlaps)
	fmt.Println(yOverlaps)
	fmt.Println(zOverlaps)
