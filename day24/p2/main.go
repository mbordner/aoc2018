...
type Army struct {
	...
	groups     GroupMap
	groupCount int
	origUnits  map[*Group]int
	origAp     map[*Group]int
}

func NewArmy(name string) *Army {
	return &Army{name: name, groups: make(GroupMap), origUnits: make(map[*Group]int), origAp: make(map[*Group]int)}
}

func (a *Army) Reset() {
	for g, units := range a.origUnits {
		g.units = units
		g.ap = a.origAp[g]
		a.groups[g] = true
	}
...
func (a *Army) AddGroup(group *Group) {
	a.origUnits[group] = group.UnitsAlive()
	a.origAp[group] = group.ap
	a.groups[group] = true
	a.groupCount++
}
...
func tryBoost(armies Armies, boost int) bool {
	armies.Reset()
	armies[0].BoostAP(boost)
	for armies.AllGroupsHaveUnits() {
		armies.Fight()
	}
	if armies[0].GetGroupsAliveCount() > 0 {
		return true
	}
	return false
}
...
func main() {
	...
	// did a manual binary search, 33, 32 get into a situation where the remaining unit can't be killed
	// would need a way to break out, but i could easily see the answer, so didn't implement :)
	fmt.Println(tryBoost(armies, 34))
